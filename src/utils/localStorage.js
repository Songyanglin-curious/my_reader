...
export function setBookObject (fileNmae, key, value) {
    let book = getLocalStorage(`${fileNmae}-info`)
    if (!book) {
        book = {}
    }
    book[key] = value
    setLocalStorage(`${fileNmae}-info`, book)
}
export function getBookObject (fileNmae, key) {
    let book = getLocalStorage(`${fileNmae}-info`)
    if (book) {
        return book[key]
    } else {
